...
@section('content')
  <div class="flex items-center justify-between mb-6">
    ...
    <a href="{{ route('admin.crew.create') }}" class="bg-[#D0D6F9] text-black px-4 py-2 rounded">
      Nouveau membre
    </a>
  </div>
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          @foreach($members as $m)
            @php
              // Compat : is_published OU published suivant la structure de la table
            ...
            <tr class="border-t border-white/10">
              ...
              <td class="px-4 py-3 text-right">
                <a href="{{ route('admin.crew.edit', $m) }}"
                   class="px-3 py-1 rounded border border-white/20 mr-2">
                  Éditer
                </a>

                <form action="{{ route('admin.crew.destroy', $m) }}"
                      method="POST"
                      class="inline"
